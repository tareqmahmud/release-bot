 ...
 * @param {Object} payload - GitHub webhook payload or normalized release event
 * @param {string|null} overrideChatId - Optional chat ID override
 * @param {string} source - Source of the event ('webhook' or 'polling')
 */
export async function sendReleaseNotification(payload, overrideChatId = null, source = 'webhook') {
  ...
  try {
    ...
    if (!releaseData.body || releaseData.body.trim().length === 0) {
      ...
      try {
        ...
        const fullRelease = await fetchReleaseDetails(release.id, repository.owner.login, repository.name);
        if (fullRelease.body && fullRelease.body.trim().length > 0) {
          ...
          const previousRelease = await fetchPreviousRelease(releaseData.tagName, repository.owner.login, repository.name);
          if (previousRelease) {
